...
public final class TodoItemsClient {
     ...
     */
    @Metadata(generated = true)
    public CreateJsonResponse createJson(TodoItem item, List<BinaryData> attachments) {
    ...
    public CreateFormResponse createForm(ToDoItemMultipartRequest body) {
        ...
        RequestOptions requestOptions = new RequestOptions();
        return createFormWithResponse(new MultipartFormDataHelper(requestOptions)
            .serializeJsonField("item", body.getItem())
            .serializeFileFields("attachments",
                body.getAttachments() == null
                    ? null
                    : body.getAttachments().stream().map(TodoFileAttachment::getContent).collect(Collectors.toList()),
                body.getAttachments() == null
                    ? null
                    : body.getAttachments()
                        .stream()
                        .map(TodoFileAttachment::getContentType)
                        .collect(Collectors.toList()),
                body.getAttachments() == null
                    ? null
                    : body.getAttachments().stream().map(TodoFileAttachment::getFilename).collect(Collectors.toList()))
            .end()
